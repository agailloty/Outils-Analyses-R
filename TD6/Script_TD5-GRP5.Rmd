...
```{r}
table(mental_health$HAS_INTERNET)
```

```{r}
sum(mental_health$HAS_INTERNET == 1)
```

```{r}
table(mental_health$REGION)
```
...
On peut décrire les cluster par composantes ou par la fréquence des individus dans chaque modalité.

```{r}
classif$desc.axes
```

Probabilité d'appartenir au cluster 1 sachant qu'on a répondu DEPRESSION = 0 => Cla/Mod => P(Cluster=1 / DEPRESSION =0)
Probabilité d'avoir déclaré ne pas être dépressif sachant qu'on appartient au cluster 1 => Mod/Cla => P(DEPRESSION =0 / Cluster=1) 
Probailité d'avoir répondu DEPRESSION=0 => Global => P(DEPRESSION = 0)

La p.value et le v.test sont liés. 
Si le v.test > |2.96| alors le lien est significatif. 
Le signe du v.test indique la sous représentation ou la surreprésentation de la modalité dans la classe.
...
```{r}
classif3 <- HCPC(seconde_acm, nb.clust = 4, graph = FALSE)
```


```{r}
fviz_cluster(classif3)
```
