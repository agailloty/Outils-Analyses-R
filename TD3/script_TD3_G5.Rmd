...
# ACP avec FactoMineR
C'est dans le package FactoMineR que sont définies la plupart des fonctions qu'on va utiliser pour faire les analyses factorielles. 
...
```{r}
premiere_acp <- PCA(X = fifa_100, quanti.sup = idx_var_quanti_illustratives, quali.sup = idx_var_quali_illustratives, graph = FALSE)
```
...
1.3 COMBIEN DE COMPOSANTES RETENIR ? 

```{r}
premiere_acp$eig
```
L'objet eig nous renvoie un tableau qui nous permet de déterminer combien de composantes retenir dans l'ACP.
...
On peut grâce à l'ACP réduire la dimensionalité d'un jeu de donnée. 
On avait 19 colonnes dans le jeu de donnée intial mais grâce à 4 variables synthétiques (dimensions, composantes) on arriver à extraire 87% de l'information totale.
...
```
La variable goalkeeping_handling contribue à hauteur de 9.53% à former l'axe 1.
...
Si on fait la somme de toutes les contributions des variables sur chaque axe on trouve 100.
...
On aimerait voir le signe de chaque variable dans la construction des axes. 
...
Sur la dimension 2 (composante 2) les variables power_strength et defending_sliding_tackle ont des coordonnées positives. Si on devrait les représenter sur un espace orhtonomé ces variables seraient à droite.
...
On appelle ce graphique le cercle des corrélations. 
Il représente les corrélations de chaque 

La fonction plot.PCA prend en argument l'objet ACP qu'on a créé, puis les axes qu'on veut représenter et l'argument choix prend en valeur : "var", "ind".
...
On explore toujours les propriétés de l'objet premiere_acp et on regarde dans les individus. 
Si on affiche le tableau global on a les coordonnées de chaque joueur sur chaque axe (dimension , composante).


```{r}
sort(premiere_acp$ind$coord[, 1], decreasing = TRUE)[1:10]
```
N, Kante est le plus à droite de l'axe 1, a la plus grande coordonnée positive. 
...
La dimension 1 explique 50% de la variance du jeu de données. 

```{r}
factoextra::fviz_screeplot(premiere_acp, title = "Pouvoir explicatif de chaque composante")
```

### Représenter le cercle de corrélation puis mettre en avant la contribution de chaque variable 

```{r fig.height=6, fig.width=10}
factoextra::fviz_pca_var(premiere_acp, axes = c(1,2), col.var="contrib", title= "Cercle de corrélation avec contribution des variables")
```

### Représenter le % de contribution de chaque variable sur les axes 1 et 2 

la fonction fviz_contrib montre le pourcentage de contribution de chaque variable sur l'axe choisi.

```{r}
fviz_contrib(premiere_acp, axes = 1, choice = "var")
```
```{r}
fviz_contrib(premiere_acp, axes = 2, choice = "var")
```

# CLASSIFICATION ASCENDANTE HIERARCHIQUE 

La classification ascendante hiérarchique nous permet de regrouper des joueurs qui ont des caractéristiques similaires dans un même groupe qu'on appelle cluster. 
On choisit un nombre de cluster à produire et la CAH fait en sorte que chaque cluster soit différent d'un autre mais qu'au sein de chaque cluster les individus se ressemblent. 

### CLASSER LES JOUEURS DANS 5 CLUSTERS

On va utiliser une fonction qui vient du package FactoMineR qui s'appelle HCPC
on lui donne en agument l'objet ACP qu'on a créé puis on précise le nombre de cluster qu'on veut créer.

Pour laisser la classification déterminer le nombre optimal de classe, précisez nb.clust = -1.


```{r}
library(FactoMineR)
classsif <- HCPC(premiere_acp, nb.clust = 5, graph = FALSE)
```
## QUELLE(S) COMPOSANTE(S) CARACTÉRISENT LES PLUS LES INDIVIDUS DE CHAQUE CLUSTER ? 
Il nous crée un objet classif qui contient tous les résultats de la classification. 

La propriété desc.axes renvoie une description de chaque cluster

Pour la classification, l'algrithme a déterminé qu'il est optimal de ne retenir que 3 dimensions pour différencier les joueurs et les classer dans chaque cluster.
...
2.3 QUELS INDIVIDUS SONT LES PLUS REPRÉSENTATIFS DANS CHAQUE CLUSTER ? 


```{r}
library(dplyr)
cluster1 <- filter(classsif$data.clust, clust ==1)
row.names(cluster1)
```

```{r}
cluster2 <- filter(classsif$data.clust, clust ==2)
row.names(cluster2)
```
...
```{r}
classsif$desc.ind$para
```



2.4 QUELS INDIVIDUS SONT LES MOINS REPRÉSENTATIFS DANS CHAQUE CLUSTER ?

Les individus spécifiques. 
Ce sont les individus le splus éloignés du centre du cluster.

```{r}
classsif$desc.ind$dist
```
...
library(factoextra)
fviz_cluster(classsif, repel = TRUE)
```

# Utiliser une interface graphique pour réaliser une ACP

On peut utiliser Factoshiny directement sur le jeu de données.
...
Ou bien l'utiliser sur une ACP qu'on a déjà réalisée.

```{r}
Factoshiny(premiere_acp)
```
